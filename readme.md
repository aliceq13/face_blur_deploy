...
3. **인물별 클러스터링**: 영상 전체에서 등장하는 인물을 식별하고 그룹화 (Re-ID)
...
```mermaid
graph TD
    User[사용자] -->|업로드/요청| Django[Django Web Server]
    Django -->|메타데이터| DB[(PostgreSQL/SQLite)]
    Django -->|작업 요청| Redis[Redis Broker]
    Redis -->|작업 분배| Celery[Celery Worker (AI)]
    Celery -->|얼굴 분석/블러링| Models[Local AI Models]
    Celery -->|결과 저장| Storage[Local/S3 Storage]
```
